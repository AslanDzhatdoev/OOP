...
function clearGraph() {
    ...
    img.src = '';
    img.style.display = 'none'; // ⬅️ Скрываем картинку
...
async function plotGraph(plotType) {
    ...
    if (response.ok) {
        ...
        const img = document.getElementById('graph_image');
        img.style.display = 'none'; // Скрыть заранее

        img.onload = () => {
            img.style.display = 'block'; // Показать только после загрузки
            updateAccordionHeights(); // Пересчитать высоту
        };

        img.onerror = () => {
            console.error("График не загрузился.");
        };
